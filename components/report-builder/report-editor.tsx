...
${localReport.sections.nextSteps}
  ...
  return (
    <div className="report-editor flex flex-col h-full border rounded-lg overflow-hidden bg-white shadow-sm">
      <div className="p-4 border-b bg-slate-50 flex justify-between items-center">
        <div>
          <h2 className="text-lg font-medium">4-Box Report</h2>
          <p className="text-sm text-slate-500">Status report based on your documents</p>
        </div>
        <div className="flex gap-2">
          <button 
            className="flex items-center gap-1 py-2 px-3 rounded-md hover:bg-slate-200 text-slate-600 text-sm border border-slate-300"
          ...
          <button 
            className={`flex items-center gap-1 py-2 px-3 rounded-md ${
              copied 
                ...
                : 'hover:bg-slate-200 text-slate-600 border border-slate-300'
            }`}
      ...
      <div className="flex-1 p-4 overflow-auto">
        ...
        <div className="border-b pb-4 mb-4">
          <input 
            type="text"
            className="w-full text-xl font-semibold border-0 focus:outline-none focus:ring-1 focus:ring-blue-500 p-1 rounded"
            placeholder="Project or Team Name"
            value={localReport.title}
            onChange={(e) => handleTitleChange(e.target.value)}
          />
          <div className="flex justify-end text-slate-500 text-sm items-center mt-2">
            <Calendar className="h-4 w-4 mr-1" />
            <input
              type="text"
              className="text-right border-0 focus:outline-none focus:ring-1 focus:ring-blue-500 p-1 rounded bg-transparent text-slate-500 text-sm w-40"
        ...
        <div className="space-y-4">
          {/* Accomplishments */}
          <div 
            className={`border rounded-md overflow-hidden shadow-sm ${activeSection === 'accomplishments' ? 'ring-2 ring-blue-500' : ''}`}
          ...
          >
            ...
            <div className="p-3 bg-white">
              <textarea 
                className="w-full border-0 focus:outline-none focus:ring-0 p-0 text-sm min-h-[120px] resize-none" 
          ...
          {/* Insights */}
          <div 
            className={`border rounded-md overflow-hidden shadow-sm ${activeSection === 'insights' ? 'ring-2 ring-blue-500' : ''}`}
          ...
          {/* Decisions */}
          <div 
            className={`border rounded-md overflow-hidden shadow-sm ${activeSection === 'decisions' ? 'ring-2 ring-blue-500' : ''}`}
          ...
          {/* Next Steps */}
          <div 
            className={`border rounded-md overflow-hidden shadow-sm ${activeSection === 'nextSteps' ? 'ring-2 ring-blue-500' : ''}`}
